...
public class Shrink extends RareProperty {
    ...
    public Shrink(RareProperties plugin) {
        super(plugin, new ArrayList<>(), "shrink", RareProperties.propertyConfig.shrinkName);
        setDescription(RareProperties.propertyConfig.shrinkDescription);
    }
    ...
    public void applyEffect(Player player, Event event) {
        ...
        if (random > getLevel() * RareProperties.propertyConfig.shrinkChance) {
            return;
        }

        LivingEntity damaged = (LivingEntity) ((EntityDamageByEntityEvent) event).getEntity();
        damaged.getAttribute(Attribute.SCALE).setBaseValue(0.5);
        player.playSound(player.getLocation(), Sound.valueOf(RareProperties.propertyConfig.shrinkSound), 1F, 1F);
